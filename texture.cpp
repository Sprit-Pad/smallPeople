...
//
static std::map<wstring, ComPtr<ID3D11ShaderResourceView>> resources;
	...
	D3D11_TEXTURE2D_DESC* texture2d_desc)
{
	...
	else
	{
		...
		{
			Microsoft::WRL::ComPtr<ID3D11DeviceContext> immadate_context;
			device->GetImmediateContext(immadate_context.GetAddressOf());
			//hr = CreateDDSTextureFromFile(device, dds_filename.c_str(), resource.GetAddressOf(), shader_resource_view);
			hr = CreateDDSTextureFromFile(device,immadate_context.Get(), dds_filename.c_str(),resource.GetAddressOf(),shader_resource_view );
			_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
		}
		else
		{
